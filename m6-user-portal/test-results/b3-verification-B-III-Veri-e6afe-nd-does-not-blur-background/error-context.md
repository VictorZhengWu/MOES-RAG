...
Locator: locator('[id^="msg-"]').last().getByText(/\[1\]/)
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('[id^="msg-"]').last().getByText(/\[1\]/)
...
- button "Copy"
- paragraph: What are DNV requirements for LNG cargo tank structures?
- button "Attach file"
...
> 135 |     await expect(badge1).toBeVisible({ timeout: 15000 });
  ...
  221 |     await page.goto('/en/chat');
  222 | 
  223 |     // Send first question
  224 |     const input = page.getByPlaceholder(/Ask anything/);
  225 |     await input.fill('First question about DNV');
  226 |     await input.press('Enter');
  227 |     await expect(page.locator('.prose').first()).toBeVisible({ timeout: 15000 });
  228 | 
  229 |     // Send second question
  230 |     await input.fill('Second question about ABS');
  231 |     await input.press('Enter');
  232 |     await expect(page.locator('.prose').first()).toBeVisible({ timeout: 15000 });
  233 | 
  234 |     // Jump nav lines — fixed positioned on right edge
  235 |     const jumpLines = page.locator('.fixed button.rounded-full');
```