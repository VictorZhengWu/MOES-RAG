...
**描述**: 在 Deep Research 报告中点击任意结论，可以展开推导过程并提出质疑。

**验收标准**:
- [ ] 报告中每个结论可点击 → 展开面板: 原始检索结果 + 推理步骤 + 引用原文
- [ ] "质疑"按钮 → 输入疑点 → 调用 `POST /research/{id}/question` → AI 重新分析 → 更新结论
- [ ] 质疑过程追加到报告末尾（标注"人工复核"）
- [ ] M8 路由已预留，需实现 M5 的后端逻辑
...
| 00107-02 | M6 Issues Kanban 拖放 (HTML5 DnD + 乐观更新) | M6 |
| 00107-03 | M5 合规自动更新 (conclusion citation → compliance needs_review) | M5 |
| 00107-04 | M5+M6 Deep Research 质疑深入 (FR-7) | M5/M6 |
...
```
P0: 00106-01 → 00106-02         (串行)
P1: 00107-01 ‖ 00107-02 ‖ 00107-03 ‖ 00107-04  (全部可并行)
P2: 00108-01 ‖ 00108-02 ‖ 00108-03 ‖ 00108-04  (全部可并行)
