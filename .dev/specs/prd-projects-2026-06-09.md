...
```
┌─────────────────────────────────────────────────┐
│ ← 返回项目列表    Ballard 82000 DWT BC    [⚙设置]│
...
| 00105-02 | `conversations.py`: 对话关联 + 文件夹 + 标签 | M5 |
| 00105-03 | `documents.py`: 文档上传 + M1 集成 + 版本管理 | M5/M1 |
| 00105-04 | `issues.py` + `conclusions.py`: 待研究问题 + 结论 | M5 |
| 00105-05 | `compliance.py`: 合规矩阵 + 偏差清单 | M5 |
| 00105-06 | `search_scope.py`: 项目范围搜索 | M5 |
| 00105-07 | M6 项目列表 + 详情页 | M6 |
| 00105-08 | M6 文件夹树 + 对话面板 + 文档管理 | M6 |
| 00105-09 | M6 问题看板 + 合规面板 + Deep Research 集成入口 | M6 |
| 00105-10 | 测试 + 集成验证（含 Phase 4-A 集成测试） | M5/M6 |
...
01 → 02 → 03 → 04 → 05 → 06
                         ...
                         07 → 08 → 09 → 10
```
(01-06 M5 后端可先做，07-09 M6 前端依赖后端 API)
