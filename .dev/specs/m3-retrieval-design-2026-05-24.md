...
- **管道化设计** — 7 阶段管线：查询分析 → 并行双路检索 → 融合 → 重排序 → 去重 → 上下文扩展 → 输出
...
│   │   └── context_expander.py  # 阶段 6：父文档上下文扩展
│   │
│   ├── embeddings/
│   │   ├── __init__.py
...
阶段 1: 查询分析 (query_analyzer.py)
    ...
    │       │
    │       ⚡ 阶段 2a 和 2b 通过 asyncio.gather 并行执行
    │       │  预期延迟: 50-100ms（取决于索引大小）
    │
...
阶段 4: 重排序 (reranker.py)
    ...
    │
    ▼
阶段 5: 去重 (pipeline.py 内联)
    │  相似度阈值: 0.85
    │  方法: 比较相邻 chunk 的文本 Jaccard 相似度
    │  跳过高度相似（同文档同段落）的冗余 chunk
    │  保留第一个，标记后续为 duplicate
    │
    ▼
阶段 6: 上下文扩展 (context_expander.py)
...
减少重复查询的延迟。20-30% 的用户查询是重复的（规范查阅场景）。

```python
@dataclass
class RetrievalConfig:
    enable_cache: bool = True
    cache_ttl: int = 3600       # 1 小时过期
    cache_max_size: int = 1000  # 最多缓存 1000 条
```

实现：内存 LRU 缓存，key = `md5(query + filters_str)`。缓存命中时直接返回，跳过全部管线（<1ms 响应）。

## 3.2 自适应快速路径

根据查询特征自动选择管线深度，避免不必要的计算开销。

```python
async def retrieve(self, request: RetrievalRequest):
    # 快速路径 1: 精确匹配查询（规范编号、文档名）
    if _is_exact_match(request.query):
        return await self._bm25_only(request)       # < 50ms

    # 快速路径 2: 关键词查询（"EH36 温度"）
    if _is_keyword_query(request.query):
        return await self._hybrid_no_rerank(request)  # < 150ms

    # 标准路径: 完整 7 阶段
    return await self._full_pipeline(request)          # < 500ms

def _is_exact_match(query: str) -> bool:
    """检测是否为规范编号/文档名格式 (e.g. 'DNV-Pt4-Ch3-2024')"""
    return bool(re.match(r'^[A-Z]{2,5}[- ]Pt\.?\d+[- ]Ch\.?\d+', query))

def _is_keyword_query(query: str) -> bool:
    """检测是否为关键词组合（<5 词，含专有名词）"""
    words = query.split()
    return len(words) <= 5 and any(w.isupper() for w in words)
```
